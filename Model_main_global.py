...
def calc_trunc_norm_param2(lowerbound, upperbound, cap_1, cumprob_likely, mean):
    def fn(x):
        root1 = (
            x[0] + x[1] * 
...
def sample_stochastic_params(N):
...
def generate_stochastic_samples(operational_samples, possible_samples, likely_samples, cumprob_likely, likely2025_mean, N):
    ...
    for i in range(N):
        ...
        dist_params = calc_trunc_norm_param2(lowerbound_capacity, upperbound_capacity, cap_1, cumprob_likely, likely2025_mean)
...
def calc_technology_diffusion(demand_jet_fuel, initial_capacity_samples, growth_rate_samples, delta_t, start_year, end_year):
    results = []
...
status_capacity = "Announced_capacity_by_status.csv"
...
operational_samples, possible_samples, likely_samples = sample_stochastic_params(N)
...
initial_capacity_samples, growth_rate_samples = generate_stochastic_samples(
    operational_samples, possible_samples, likely_samples, cumprob_likely, likely2025_mean, N
)
simulation_results = calc_technology_diffusion(demand_jet_fuel, initial_capacity_samples, growth_rate_samples, delta_t, start_year, end_year)
